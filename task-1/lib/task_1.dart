void main() {
  List<AgriculturalMachinery> mach = [];

  int allAge = 0;
  int halfAge = 0;

  mapBefore2010.forEach((keys, values) {
    values.forEach((territory) {
      mach.addAll(territory.machineries);
    });
  });

  mapAfter2010.forEach((keys, values) {
    values.forEach((territory) {
      mach.addAll(territory.machineries);
    });
  });
